...
const EmoteMessage: React.FC<EmoteMessageProps> = ({ message, className }) => {
  try {
    ...
    sortedEmotes.forEach((emote, i) => {
      // Validate emote indices
      if (emote.startIndex < 0 || emote.endIndex >= message.message.length || emote.startIndex > emote.endIndex) {
        console.warn('Invalid emote indices:', emote);
        return;
      }
    ...
    console.error('Error rendering emote message:', error);
    // Fallback to plain text if emote parsing fails
    return <span className={className}>{message.message}</span>;
  }
};
